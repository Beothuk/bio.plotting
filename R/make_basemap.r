...
#' depending on the data. Check \code{\link{http://spatialreference.org/ref/epsg/}} for more options.
#' @param \code{p.plotting} a list derived from \code{bio.plotting::load_environment()} holding some default variables
#' @param \code{df} default is \code{NULL}.  You can optionally send a dataframe to this function, which can be have the plot extent match 
#' the data.
#' @param \code{auto.setlimits} default is \code{NULL}. This controls the extent of the resultant map.  If \code{auto.setlimits = T} (and a
...
#' @param \code{x.limits} default is \code{NULL} but an appropriate value would be in the form of \code{c(-70,-54)}. These are the default 
#' bounding longitudes. If unspecified, the value from p.plotting will be used.
#' @param \code{y.limits} default is \code{NULL} but an appropriate value would be in the form of  is \code{c(41,50)}. These are the default 
#' bounding latitudes for extent. If unspecified, the value from p.plotting will be used.
#' @param \code{known.areas} default is \code{NULL}, but can include any areas identified in the \code{get_known_areas()} function. If 
#' unspecified, the value from p.plotting will be used.
#' @param \code{known.areas.add} default is \code{NULL}.  This flag indicates whether or not the specified \code{known.areas} should be 
#' plotted.  If unspecified, the value from p.plotting will be used.
#' @param \code{known.areas.detailed} default is \code{NULL}.  This flag indicates whether the detailed versions of known areas should be 
...
#' @family plotting
#' @export
make_basemap = function(p.plotting=p.plotting, df=NULL, auto.setlimits=NULL, crs.out=NULL, x.limits=NULL, y.limits=NULL, 
  ...
  if (length(known.areas)>0) {
    known.areas=get_known_areas(p.plotting=p.plotting)
